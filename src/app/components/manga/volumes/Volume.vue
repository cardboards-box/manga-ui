...
<div class="volume-container" :class="{ 'collapse': volume.collapse }">
    <header class="flex" @click="toggle">
        <p class="fill">{{ volume.ordinal ? 'Volume ' + volume.ordinal : 'No Volume' }} - {{ volume.chapters.length }} Chapter{{ volume.chapters.length > 1 ? 's' : '' }}</p>
...
import type { MangaVolume, booleanish } from '~/models';
